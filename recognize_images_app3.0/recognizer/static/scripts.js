...
function createActionButtons() {
    ...
    if (isManualFolder) {
        ...
        recognizeBtn.onclick = () => {
            ...
            showRecognitionConfirmationDialog();
        };
...
// Функция для показа диалога подтверждения распознавания
function showRecognitionConfirmationDialog() {
    ...
    // Заголовок
    const title = document.createElement('h2');
    title.textContent = 'Распознавание изображений';
    title.style.marginBottom = '20px';
    
    // Сообщение
    const message = document.createElement('p');
    message.textContent = 'Все изображения из ручных папок будут обработаны нейросетью, классифицированы и распределены по категориям в папке "Папки ИИ". Продолжить?';
    ...
    confirmBtn.textContent = 'Начать распознавание';
    confirmBtn.onclick = async () => {
        // Показываем индикатор загрузки
        message.textContent = 'Идет обработка изображений, пожалуйста, подождите...';
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                confirmBtn.onclick = () => {
                    document.body.removeChild(modal);
                    // Перезагружаем структуру папок, чтобы отобразить новые папки
                    loadFolderStructure();
                };
            } else {
                message.textContent = 'Ошибка при распознавании: ' + (data.message || data.error || 'Неизвестная ошибка');
            ...
            message.textContent = 'Ошибка при распознавании: ' + error.message;
            confirmBtn.textContent = 'Закрыть';
            confirmBtn.disabled = false;
            confirmBtn.onclick = () => {
                document.body.removeChild(modal);
